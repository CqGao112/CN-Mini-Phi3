...
# DATA_ROOT = "/hy-tmp/"
from safetensors.torch import load_file, save_file as save_f
...
def generate_alpaca_gpt4_reject_response(groups_cnt: int = 50000, max_len: int = 320, batch_size: int = 32) -> None:
    ...
    # # 加载原始权重文件
    weights = load_file("/data1/gcq/dataset/fine_tuned/sft_merged2/model.safetensors")
    # new_weights = {"model." + key: value for key, value in weights.items()}

    # 保存为新的 .safetensors 文件
    # save_f(new_weights, "/data1/gcq/dataset/fine_tuned/sft_merged2/new_model.safetensors")
    # return
    chat = LLM(model="/data1/gcq/dataset/fine_tuned/sft_merged2/")
    batch_size = 32
